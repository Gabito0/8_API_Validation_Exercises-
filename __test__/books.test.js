...
beforeEach(async () => {
  ...
  books_isbn = results.rows[0].isbn;
});
...
describe("GET /books/:isbn", function () {
  test("Gets a single book", async function () {
    const response = await request(app).get(`/books/${books_isbn}`);
    console.log(response.body);
    expect(response.body.book).toHaveProperty("isbn");
    expect(response.body.book.isbn).toBe(books_isbn);
  });
...
describe("PUT /books/:id", function () {
  test("Updates a single book", async function () {
    const response = await request(app).put(`/books/${books_isbn}`).send({
      amazon_url: "https://taco.com",
      author: "mctest",
      language: "english",
      pages: 1000,
  ...
  test("Responds 404 if can't find book in question", async function () {
    // Delete book first
    await request(app).delete(`/books/${books_isbn}`);
